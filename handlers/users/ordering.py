...
from keyboards.default import tolov_uz, tolov_eng, amount_menu_uz, amount_menu_eng, products_menu_uz, products_menu_eng, \
...
async def order_place_eng(message: types.Message, state: FSMContext):
    ...
    if status:
        ...
        if len(products) > 0:
            ...
            await message.answer("🗑 Ваша корзина пуста, чтобы сделать заказ выберите продукты",
                                 reply_markup=products_menu_eng)
